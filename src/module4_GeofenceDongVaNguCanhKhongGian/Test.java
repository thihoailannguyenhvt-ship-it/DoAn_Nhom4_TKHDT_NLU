...
public class Test {
    ...
    static class DuLieuM1 {
        String idConVat; double viDo, kinhDo;
        DuLieuM1(String id, double v, double k) { this.idConVat = id; this.viDo = v; this.kinhDo = k; }
    }

    static class DuLieuM2 {
        String idConVat, trangThai; double odba, tyLe;
        DuLieuM2(String id, String tt, double o, double tl) { this.idConVat = id; this.trangThai = tt; this.odba = o; this.tyLe = tl; }
    }
    ...
    public static void main(String[] args) {
        ...
        try (PrintWriter txt = new PrintWriter("Data//Output//KetQuaModule4.txt");
            ...
            for (DuLieuM2 m2 : duLieuModule2) {
                ...
                if (hangDoi == null || hangDoi.isEmpty()) continue;
                DuLieuM1 m1 = hangDoi.poll();

                // Bước A: Gọi hệ thống xử lý logic
                trungTam.xuLySuKien(m2.idConVat, new Diem(m1.kinhDo, m1.viDo), m2.odba, NhanTrangThai.valueOf(m2.trangThai), 0.0);
                
                // Bước B: Lấy lại đối tượng Geofence đã được cập nhật từ kho
                GeofenceDong g = kho.timGeofenceTheoId(m2.idConVat);
                
                // Bước C: Tìm vùng giao thoa dựa trên bán kính đã tính toán chuẩn xác từ GeofenceDong
                // Sử dụng phương thức: g.getBanKinhRInDegrees(0.0)
                ...
                for(VungKhongGian v : danhSachVungGiaoThoa) {
                    ...
                    if(pt > maxPt) { maxPt = pt; bestVung = v; }
                }
                ...
                // Bước D: Đánh giá nguy cơ (Sử dụng đúng phương thức bạn đã định nghĩa)
    ...
    private static Map<String, Queue<DuLieuM1>> docModule1(String path) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(path))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                map.get(id).add(new DuLieuM1(id, Double.parseDouble(p.get(1).trim()), Double.parseDouble(p.get(2).trim())));
            }
        } catch (Exception e) { System.out.println("Loi M1: " + e.getMessage()); }
        return map;
    }
    ...
    private static List<DuLieuM2> docModule2(String path) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(path))) {
            ...
            while ((line = br.readLine()) != null) {
                if (line.trim().isEmpty()) continue;
                List<String> p = tachCSV(line, line.contains(";") ? ';' : ',');
                ds.add(new DuLieuM2(p.get(0).trim(), p.get(1).trim(), Double.parseDouble(p.get(2).trim()), Double.parseDouble(p.get(3).trim())));
            }
        } catch (Exception e) { System.out.println("Loi M2: " + e.getMessage()); }
        return ds;
    }

    private static List<String> tachCSV(String line, char delimiter) {
        List<String> kq = new ArrayList<>(); StringBuilder sb = new StringBuilder(); boolean inQ = false;
        for (char c : line.toCharArray()) {
            if (c == '"') inQ = !inQ;
            else if (c == delimiter && !inQ) { kq.add(sb.toString()); sb.setLength(0); }
            else sb.append(c);
        }
        kq.add(sb.toString()); return kq;
    }
}