 ...
 *
 * Phối hợp 3 thành phần theo thứ tự ưu tiên:
 * [A] BoXuLyNguong      – xét độc lập nhãn hành vi
 * [B] BoXuLyNguongVung  – xét độc lập nhãn không gian
 * [C] MaTranRuiRo       – kết hợp M2 × M4 khi A và B chưa đủ thuyết phục
 */
public class BoDieuPhoiTrungTam {
    ...
    public BoDieuPhoiTrungTam(BoXuLyNguong     hanhVi,
                               BoXuLyNguongVung khongGian,
                               MaTranRuiRo      maTran) {
     ...
     *
     * Cả 2 luồng [A] và [B] LUÔN chạy đủ – không được ngắt giữa chừng.
     * Điều này đảm bảo cửa sổ thời gian của cả hành vi lẫn vùng đều
     * được cập nhật liên tục, không bị bỏ sót bản ghi.
     */
    public KetQuaDanhGia xuLyGoiTin(BanGhiSinhHoc goiTin) {
        ...
        if (hanhViGuiM5 || khongGianGuiM5) {
            ...
            if (hanhViGuiM5 && khongGianGuiM5) {
                KetQuaDanhGia kqGoc = (kqHanhVi.getMucDoNghiemTrong().ordinal()
                            >= kqKhongGian.getMucDoNghiemTrong().ordinal())
                    ? kqHanhVi : kqKhongGian;
                
                // BỔ SUNG: Khởi tạo thực thể mới từ kết quả gốc để bảo vệ tính toàn vẹn dữ liệu
                KetQuaDanhGia kqKetThuc = new KetQuaDanhGia(kqGoc.getTrangThai(), kqGoc.getHanhDong(), kqGoc.getMucDoNghiemTrong(), kqGoc.getThongBao());
                // BỔ SUNG: Sử dụng chính xác hàm capNhatThoiGianSuKien theo class KetQuaDanhGia của bạn
        ...
        // BỔ SUNG: Khởi tạo thực thể mới từ kết quả ma trận để đóng dấu thời gian riêng biệt cho bản ghi này
        KetQuaDanhGia kqMaTranKetThuc = new KetQuaDanhGia(kqMaTranGoc.getTrangThai(), kqMaTranGoc.getHanhDong(), kqMaTranGoc.getMucDoNghiemTrong(), kqMaTranGoc.getThongBao());
        // BỔ SUNG: Sử dụng chính xác hàm capNhatThoiGianSuKien theo class KetQuaDanhGia của bạn
