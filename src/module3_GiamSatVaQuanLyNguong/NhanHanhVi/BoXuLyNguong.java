 ...
 *
 * FIX #1: Bỏ khoLuuTruHoSo (BanGhiSinhHoc) – không cần thiết vì BoDanhGiaHanhVi
 * đã tự quản lý cửa sổ trượt theo idCaThe. Giữ một Map thứ 2 chỉ để track nhãn
 * cuối cùng phục vụ "chống nhiễu".
 *
 * FIX #2: Logic dọn dẹp sau DA_XAC_NHAN được sửa lại: Chỉ xóa khi hành động là
 * GUI_THANG_M5 (thay vì kiểm tra hanhDong != DANG_THEO_DOI – điều kiện cũ không
 * bắt đúng case).
 */
public class BoXuLyNguong {
	...
	public KetQuaDanhGia xuLy(BanGhiSinhHoc goiTin) {
		...
		// Chọn chiến lược
		ChienLuocDanhGia chienLuoc = danhMucChienLuoc.getOrDefault(trangThaiMoi, new ChienLuocMacDinh());
